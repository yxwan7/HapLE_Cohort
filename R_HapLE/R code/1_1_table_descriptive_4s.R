...
sample_breakdown <- follow_up_sample_desc %>%
  ...
  mutate(
    ...
    is_neutral = ifelse(H == 2, 1, 0),
    is_unhappy = ifelse(H == 3, 1, 0),
  ...
  summarize(
    ...
    pct_neutral = mean(is_neutral, na.rm = TRUE) * 100,
    pct_unhappy = mean(is_unhappy, na.rm = TRUE) * 100,
...
result_table <- full_breakdown_pct %>%
  select(
    ...
    pct_urban, pct_literate, pct_happy, pct_neutral, pct_unhappy, pct_disabled
  )
...
comparison_table <- cohort1_stats_final %>%
  ...
  select(
    ...
    pct_neutral_c1, pct_neutral_c2,
    pct_unhappy_c1, pct_unhappy_c2,
...
# Save as CSV
write.csv(result_table, "R output/1_1_descriptive_cohort_4s.csv", row.names = FALSE, na = "")
write.csv(comparison_table, "R output/1_1_descriptive_age_4s.csv", row.names = FALSE, na = "")

...
#-----------------------------------------------------
# Set the data path
file_68_73 <- "../SAS_HapLE_4s/Output/LE/sex+urban/LE_68_73_sex+urban_4s_r.txt"
file_74_79 <- "../SAS_HapLE_4s/Output/LE/sex+urban/LE_74_79_sex+urban_4s_r.txt"
file_80_85 <- "../SAS_HapLE_4s/Output/LE/sex+urban/LE_80_85_sex+urban_4s_r.txt"
file_86_91 <- "../SAS_HapLE_4s/Output/LE/sex+urban/LE_86_91_sex+urban_4s_r.txt"
...
raw_data <- combined_df %>%
  ...
  mutate(
    Residence = factor(urban, levels = c(1, 2), labels = c("Urban", "Rural")),
...
cohort_results_les <- cohort_results_base %>%
  ...
  select(
    `Age Range` = AgeGroup, Residence, Cohort = Cohort_Label,
...
difference_results_les <- difference_results_base %>%
  ...
  mutate(Cohort = "Diff.") %>%
  select(`Age Range`, Residence, Cohort, everything()) # Ensure correct column order
...
combined_results_les <- bind_rows(
  ...
  arrange(`Age Range`, Residence, Cohort)
...
caption_les <- paste("Life Expectancy by Residence: Estimates, Differences, and 95% Confidence Intervals.", caption_text_base)
...
# Export HTML tables
filename_les_html <- "R output/1_3_life_expectancy_urban_4s.html"

save_kable(table_les_kable, file = filename_les_html, self_contained = TRUE)

# Export LaTeX tables
filename_les_latex <- "R output/1_3_life_expectancy_urban_4s.tex"

save_kable(table_les_latex, file = filename_les_latex)

# Export underlying data frames to CSV
write.csv(combined_results_les, "R output/1_3_life_expectancy_urban_4s.csv", row.names = FALSE)

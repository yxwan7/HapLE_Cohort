# ==============================================================================
# TITLE:     LE estimates by residence
# DATE:      2025-07-05
# ==============================================================================
...
cohort_results_les <- cohort_results_base %>%
  ...
  select(
    `Age Group` = AgeGroup, Residence, Cohort = Cohort_Label,
...
difference_results_les <- difference_results_base %>%
  ...
  mutate(Cohort = "Diff.") %>%
  select(`Age Group`, Residence, Cohort, everything()) # Ensure correct column order
...
combined_results_les <- bind_rows(
  ...
  arrange(`Age Group`, Residence, Cohort)
...
# Export HTML tables
filename_les_html <- "R output/1_4_life_expectancy_urban.html"

save_kable(table_les_kable, file = filename_les_html, self_contained = TRUE)

# Export LaTeX tables
filename_les_latex <- "R output/1_4_life_expectancy_urban.tex"

save_kable(table_les_latex, file = filename_les_latex)

# Export underlying data frames to CSV
write.csv(combined_results_les, "R output/1_4_life_expectancy_urban.csv", row.names = FALSE)

...
cohort_results_les <- cohort_results_base %>%
  ...
  select(
    `Age Group` = AgeGroup, Education, Cohort = Cohort_Label,
...
difference_results_les <- difference_results_base %>%
  ...
  mutate(Cohort = "Diff.") %>%
  select(`Age Group`, Education, Cohort, everything()) # Ensure correct column order
...
combined_results_les <- bind_rows(
  ...
  arrange(`Age Group`, Education, Cohort)
...
# Export HTML tables
filename_les_html <- "R output/1_3_life_expectancy_edu.html"

save_kable(table_les_kable, file = filename_les_html, self_contained = TRUE)

# Export LaTeX tables
filename_les_latex <- "R output/1_3_life_expectancy_edu.tex"

save_kable(table_les_latex, file = filename_les_latex)

# Export underlying data frames to CSV
write.csv(combined_results_les, "R output/1_3_life_expectancy_edu.csv", row.names = FALSE)

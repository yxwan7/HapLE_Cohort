...
file_68_73 <- "../SAS_HapLE/Output/LE/sex+edu/LE_68_73_sex+edu_b300.txt"
file_74_79 <- "../SAS_HapLE/Output/LE/sex+edu/LE_74_79_sex+edu_b300.txt"
file_80_85 <- "../SAS_HapLE/Output/LE/sex+edu/LE_80_85_sex+edu_b300.txt"
file_86_91 <- "../SAS_HapLE/Output/LE/sex+edu/LE_86_91_sex+edu_b300.txt"
...
raw_data <- combined_df %>%
  ...
  mutate(
    Edu = factor(edu, levels = c(1, 2), labels = c("Literate", "Illiterate")),
...
plot_data_long <- raw_data %>%
  select(AgeGroup, Edu, Cohort_Label, LEs1, LEs2) %>%
  pivot_longer(
...
# Updated harmonious green color palette (Pantone-inspired)
color_palette <- c(
  "HapLE" = "#a8e6cf", # Light green (Pantone 14-6327 TPX inspired)
  "UnHapLE" = "#27ae60" # Dark green (Pantone 17-6153 TPX inspired)
)
...
#-----------------------------------------------------
# save the plot
ggsave("R output/2_3_HapLE_stacked_plots_edu.png", plot = plot, width = 8, height = 7, dpi = 300)

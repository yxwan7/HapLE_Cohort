...
/***(1) All programs */
  %LET BSIZE=10;                 /*** # of bootstrap samples. At least 0. It has to be a positive integer >= 2.*/
  %LET BEG=65;                  /*** First age of the life table.*/
  %LET AgeLowerLimit=65;                  /*** The lowest age from the input data. Range from 0 to BEG*/
  %LET Datainput= hle ;  /*** The name of SAS input data file. */
  %LET depVAR= S ;            /*** Dependent variable (Health state variable). Can NOT use "HS".*/
  %LET STRATCOV=  ;           /*** Covariate(s) used to stratify the analysis.*/
  %LET REGCOV= sex cohort;               /*** Covariate(s) in regression models. */
  ...
  %LET nHealthState=5;          /*** # of health states (including the absorbing state). Only SIM program (2+) can you use values other than 3. */
  ...
  %LET InteractionTerm =     ;   /*** User defined interaction terms. Default is blank.              ***/ 
                                                            /*** For example:                                                   ***/
                                                            /***              Age*Sex                                           ***/
                                                            /***              Age*Sex Sex*Education                             ***/
  %LET AGE_sq = 0;              /*** 1 = having an age squared term; 0, otherwise. Default = 0*/

/***(2) RAD program (module 1) only */
  %LET END_LT= 74 ;            /*** Last age (open interval) the life table.*/
  
/***(3) SIM program (module 2) only */
  %LET SIMSIZE= 10000 ;         /*** Size of simulation cohort.*/
  %LET SIMYEAR= 10 ;         /*** Year length of simulation.*/
  ...
  %LET nSession= 15 ;         /*** # of sessions for parallel computation under a multi-CPU computer.            */
  ...
  %LET SimLifeLine=  1 ;            /*** Whether to save simulation outputs: 0=No (Default). 1=Yes.                       */
  %LET CtrlCOVmodel= 0 ;            /*** Whether to use control variable(s) in regression models: 0=No (Default). 1=Yes.  */
  %LET CtrlCOVcateg=   ;            /*** Control variable(s) <categorical> in the regression model.                       */
